...
#Main Function
def to_json(isi_user, isi_pwd):
    # Get metadata for ALL datasets in the DM
    datamart_api_url = (f"https://{isi_user}:{isi+pwd}@dsbox02.isi.edu:8888/datamart-api-wm")
    ...
    #### Build dataset.json ####
    i = 0
    end = len(dataset_meta)-6 #6 files deleted or not in download that are in meta
    for filename in sorted(glob.glob("datamart-dump/*.csv")):
        ...
        variable_ids = list(
            df_var_meta.variable_id
        )  # list of all vars in dataset to loop thru
